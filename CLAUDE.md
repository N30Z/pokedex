...
- All speech runs self-hosted/offline in the backend container: faster-whisper (a CTranslate2 port of OpenAI's Whisper, forced to `language="de"` and run with `vad_filter=True` to strip the silence padding around the ~4s fixed-duration recording) for STT, Piper for German TTS — no cloud APIs. Both are lazy-loaded in `voice.py` (`get_whisper_model()`/`get_piper_voice()`) and raise `VoiceUnavailable`, turned into a 503 by the routes in `main.py`, when their model files aren't present under `/models` yet — this keeps the voice endpoints optional so the rest of the app (and `sync_schema()`/DB) is unaffected by whether the (large, not-in-image) model files have been fetched via `backend/scripts/download_models.sh`. faster-whisper (unlike Vosk) is a full seq2seq model with a much broader learned vocabulary, which matters here since Pokémon names aren't real German words a small keyword-spotting model would know.
...
- `GET /api/pokemon/{id}/display.png[?shiny=true]` serves the official artwork (`/data/artwork[-shiny]/{id}.png`, falling back to non-shiny) scaled by `render_display_image()` (ffmpeg) to exactly 240×240 and flattened onto black, cached under `MEDIA_PATH/display/`. The ~1MP alpha-channel originals are too slow for the ESP32 to decode, and ESPHome's `online_image` RGB565 has no alpha, so this is done server-side; the firmware draws it 1:1 full-screen.
- `synthesize_tts()` respells a few Pokémon names via `PRONUNCIATION_DE` before synthesis (Piper's German phonemizer reads "Pikachu" with a German "ch" as in "Bach"). It's an explicit list rather than a ch→tsch rule because many German names (Nachtara, Sichlor, …) need the German "ch"; add names there when one sounds wrong. The respelling happens before the cache key is hashed, so edits regenerate the affected cached WAVs.
...
```bash
# Fetch the German faster-whisper/Piper voice models once, into ./models (optional —
